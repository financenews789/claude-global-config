...
- §3.1 bis **Tout calcul glissant (YoY, 3 mois, 52 semaines) s'aligne sur l'axe des DATES, jamais sur la position des lignes** : `eco3min_series.pct_change_by_date(s, periods, freq)` (pandas seul, importé par les sept pipelines). `Series.pct_change(12)` décale de 12 lignes ; dès qu'une observation manque (CPIAUCSL et CPILFESL : octobre 2025 jamais publié, shutdown, ligne retirée par `get_resampled`), tous les mois suivants sortent sur 13 mois. Constaté le 18/09/2026 : `us-cpi-inflation` publiait 3,71 % pour août 2026 au lieu de 3,35 %, core CPI et composites `*_YOY` avec lui ; corrigé dans FRED simple et composite, articles, v2 (excess-cape-yield), score, régime (Brent, ICSA) et FR. Une base absente donne NaN, jamais une valeur interpolée (octobre et novembre 2026 du CPI n'auront pas de YoY). Test : `tests/test_pct_change_by_date.py`.
