...
description: "Production et révision des pages dataset Eco3min (eco3min.fr) : la page qui expose une série, en anglais par défaut (FR seul pour les séries françaises INSEE / Banque de France, paire FR/EN sur décision explicite) — séries d'une source primaire (FRED, ECB SDMX, IMF, BIS, BLS, Eurostat, Shiller, World Bank Pink Sheet, Coinbase via FRED, ENTSO-E, AGSI+, FAO…) et composites construits par Eco3min (ratios, séries réelles déflatées, indicateurs propriétaires type Net Liquidity Index, Excess CAPE Yield). Activer pour « crée la page dataset », « révise la page DGS10 », « page dataset du composite », « quel H1 pour ce ticker », « Cas A ou A' ? », « Macro Takeaway », « Historical Regimes », « Construction & Components », « eco3min_latest », « eco3min_key_stats », « eco3min_download », « data-series », « dataset_id », « /dataset/{slug}.csv », « la page dit CC BY », « quelle licence afficher », « Source terms », « notice de licence », « la source est-elle publiable en CSV ? », et pour toute page dont l'URL finit en -dataset/. SKILL.md = colonne vertébrale (§0 table des trois cas A / A' / B verbatim, règles bloquantes §1 à §15 sous leurs numéros d'origine dont §12.4 licence affichée ajouté le 17/09/2026, anti-patterns SEO, AMF, critère de publication, checklist et rappel zéro commentaire HTML verbatim) ; références dans references/ (à lire quand la section le dit) : 01 positionnement et familles de sources, 02 stratégie SEO par cas et convention de slug, 03 template HTML complet des 18 blocs et préservation byte-level, 04 sections éditoriales (Macro Takeaway, Construction, What This Index Captures, Data Quality, Historical Regimes, intro), 05 sourcing (usage prose / visuel / CSV, licence affichée = conditions réelles), maillage, canvas et shortcodes, paire FR/EN ; pas de scripts/. Doctrines : source et stratégie SEO sont deux axes orthogonaux, le H1 ne se déduit jamais de la source (PCOFFOTMUSDM est FRED mais Cas A') ; Cas A = ticker googlé en H1, A' et B = concept canonique, code interne (NET_LIQ, SP500_M2) 0 fois dans H1 / meta / focus keyword ; Cas B = sections Construction & Components + What This Index Captures + Data Quality obligatoires, Construction = formule et composantes, Methodology = pipeline, jamais dupliquées ; ordre des 18 sections immuable ; spinner « Loading data… » générique, jamais « Loading FRED data… » ; jamais FRED cité comme source unique d'un composite à composante non-FRED ; dataset_id immuable une fois fixé ; préservation byte-level en révision (shortcodes, canvas, pre Python/R) ; canvas rendu ou omis selon le script global, jamais de canvas mort ; 4 à 6 régimes datés et chiffrés ; 6 à 12 liens internes, EN→EN FR→FR, pas d'auto-référence ; une source se juge par usage, citable en prose ≠ redistribuable en CSV, et une page dataset exige un oui dans la colonne CSV de la table de sourcing-donnees-eco3min (référence 03), la whitelist plate est abandonnée le 17/09/2026 (CoinGecko, LBMA, S&P DJI, MSCI se citent mais ne se servent pas en CSV) ; la licence affichée = les conditions réelles de la source, « CC BY » ou « public domain » dans la prose seulement si la source l'est, une série « citation required » se décrit comme réutilisable avec attribution jamais sous Creative Commons, la ligne du snippet 36 ou 114 porte rights avant publication sinon le JSON-LD ment ; le H1 ne promet que ce que le CSV contient (ni « LBMA Fix » sur du Pink Sheet, ni « since 2010 » sur une série qui commence en 2014) ; langue = langue de la donnée, pas de doublage FR systématique (décision du 16/09/2026) ; AMF descriptif jamais prescriptif, vigilance renforcée Cas B ; zéro script, zéro commentaire HTML dans le contenu publié (incident Autoptimize du 27 août 2026). Hors périmètre : création du CSV, series_id, cron, clés rights du registry → pipeline-eco3min ; légalité et conditions de chaque source, gate fred_license.py → sourcing-donnees-eco3min ; études avec hypothèse et narrative → production-research-study ; pages « Every X since Y » → production-every-x-record ; charts DIB → production-chart-of-the-week ; cluster éditorial autour d'un ticker → cluster-ticker-renfort-dataset ; identité éditoriale et six règles AMF → editeur-eco3min. Combiner avec editeur-eco3min, formats-eco3min, visuels-eco3min, brand-kit-eco3min, pipeline-eco3min, sourcing-donnees-eco3min, archi-eco3min, eco3min-import-contenu-bilingue, cluster-ticker-renfort-dataset."
---
...
>
> Mise à jour du 17/09/2026 : CoinGecko et LBMA ne sont plus des sources de CSV (redistribution interdite par leurs conditions). BTC / ETH viennent de FRED `CBBTCUSD` / `CBETHUSD` (Coinbase), or / argent du World Bank Pink Sheet. Une source se juge par usage, colonne « (c) CSV » de `sourcing-donnees-eco3min/references/03-licences-sources-pipeline.md` (§12.4).
>
...
| `references/05-sourcing-maillage-technique-bilingue.md` | §12 à §15 : usage de la source (renvoi à la table de sourcing, remplace la whitelist depuis le 17/09/2026), §12.4 licence affichée, format des sources, tableau de maillage, canvas, shortcodes, cadence des pipelines, paire FR/EN | avant le bloc Sources, le maillage, le canvas, et avant toute paire FR/EN |
...
- Ajout du 17/09/2026 : CoinGecko n'est plus une famille vivante (pages BTC / ETH re-sourcées Coinbase via FRED le 17/09/2026, famille FRED, licence « citation required » déclarée). Toute famille se lit avec la colonne « (c) CSV » de la table de sourcing (§12.4).
...
- Une source se juge par usage : citable en prose ≠ redistribuable en CSV. Avant toute page dataset, la source a un **oui** dans la colonne « (c) CSV » de la table `sourcing-donnees-eco3min/references/03-licences-sources-pipeline.md` (depuis le 17/09/2026 elle remplace la whitelist §12.3). Sans ce oui, ou avec une ligne « à vérifier », NE PAS produire la page. Une source absente de la table est **web_search-vérifiée**, ajoutée à la table avec sa date, puis seulement publiée. Les sources sous licence / paywall (EPEX/Nord Pool, Caixin PMI…) ne sont pas redistribuables.

**§12.4 Licence affichée = conditions réelles de la source (ajout du 17/09/2026)**
- La page n'écrit « CC BY 4.0 » ou « public domain » que si la source l'est (FRED public domain, World Bank, OECD, FAO…). Une série FRED « copyrighted: citation required » (Chicago Fed, NY Fed, Moody's `BAA`, `T10YIE`, `RRPONTSYD`, `VIXCLS`, `PCOPPUSDM`, Coinbase `CBBTCUSD` / `CBETHUSD`…), IMF, BoE se décrit comme réutilisable **avec attribution à la source et à FRED**, jamais sous Creative Commons. Le statut se lit par `~/.claude/skills/sourcing-donnees-eco3min/scripts/fred_license.py`, jamais de mémoire.
- Avant publication, la ligne du dataset dans le snippet 36 (US) ou 114 (non-US) porte `'rights' => '<clé>'` quand la source l'exige : c'est elle qui met `license` / `usageInfo` / `copyrightHolder` dans le JSON-LD et la notice « Source terms » sous la page. Sans clé, le JSON-LD annonce CC BY 4.0 à tort (22 pages corrigées le 16/09/2026). La clé doit exister dans `eco3_source_rights()` ET dans `eco3min-data/scripts/source_rights.py` (`pipeline-eco3min` §2.6).
- Le H1, l'Overview et la Methodology ne promettent que ce que le CSV contient : pas de « LBMA Fix » sur une donnée Pink Sheet, pas de « since 2010 » sur une série qui commence en 2014, pas de « CSV, Excel » sur une page qui ne sert qu'un graphique FRED (cas `SP500`, pre-approval, corrigé le 17/09/2026).
- Test sur la page rendue : `curl -s <url> | grep -o '"license":"[^"]*"'` et la phrase de la prose disent la même chose ; une page « citation required » affiche la notice « Source terms ».
...
**Licence et source (ajout du 17/09/2026)**
- [ ] Source avec un « oui » en colonne « (c) CSV » de la table de sourcing ; statut FRED lu par `fred_license.py` ; pre-approval → pas de page dataset
- [ ] Prose et JSON-LD disent la même licence : « CC BY » / « public domain » seulement si la source l'est ; « citation required » → attribution, jamais Creative Commons ; ligne `rights` posée dans le snippet 36 / 114 quand la source l'exige
- [ ] H1, Overview, Methodology ne promettent que ce que le CSV contient (source réelle nommée, profondeur réelle, formats réellement servis)
