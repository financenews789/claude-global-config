...
Passe du 17/09/2026 — LECTURE À TAILLE FEED ET CONVERSION PAR FAMILLE DE FORME (20 cycles)
- Méthode : les PNG livrés des cycles 19, 20, 21 réduits à 375 px (largeur du feed sur téléphone) et relus ; conversion vue→upvote (`upvotes_24h / vues_24h`) calculée sur les 20 cycles du tracker et groupée par famille de forme.
- Constat 1 : à 375 px, seuls le titre et la forme survivent. Au cycle 21, la colonne de prix (le « second encodage » coché en Q1) et la killer phrase sont illisibles ; le feed a vu un bar chart classé, et la conversion (0,21 %) est celle d'un bar chart classé. Au cycle 20, 137 colonnes se lisent comme une texture et le sens du gouffre (« paid back ») n'existe qu'en ouvrant l'image (0,18 %). Au cycle 19, un écheveau (0,06 %).
- Constat 2 : conversion médiane 0,39 % pour les charts « forme dans le temps » (cycles 1, 3, 5, 7, 8, 10, 11, 12, 14, 15, 20 ; n = 11) contre 0,11 % pour les comparaisons statiques (2, 4, 6, 9, 13, 16, 18, 19, 21 ; n = 9). À ancrage fort seul : 0,39 % (n = 7) contre 0,11 % (n = 5). Aucune statique au-dessus de 0,29 %, aucune temporelle à ancrage fort sous 0,25 %. Le cycle 21 (522k vues, meilleur reach depuis le 11) est la démonstration : le sujet a porté, la forme a plafonné.
- Constat 3 : la version condensée de l'ÉTAPE 1 dans SKILL.md avait affaibli la règle « série historique longue, minimum 30 ans » de la référence 01 en « si le format en dépend ». Le tracker confirme la règle d'origine (les séries longues « performent 3-5× mieux » : mesuré 3,5× en conversion). Réalignée.
- Constat 4 : les templates anglais de la référence 05 (hook, deux lectures, question finale, redirect zone 3) contenaient des tirets cadratins alors que l'ÉTAPE 8 les interdit dans tout contenu public ; les trois variantes de titre proposaient un chiffre en tête et une question pure, tous deux dans les anti-patterns. Corrigés.
Apprentissages cristallisés : TEST FEED 375 px avec `feed_preview` / `feed_metrics` (ÉTAPE 3, référence 03) ; Q1 durci (un encodage est visuel ou n'est pas) ; conversion vue→upvote dans `dashboard.py cotw` et l'ÉTAPE 9 ; règle « dans l'actu n'est pas un ancrage » (référence 01) ; deux risques distincts sur le titre et bloc mécanisme pour les sujets d'agence statistique (référence 05) ; mécanisme de la métrique en toutes lettres sur l'image et dans le comment.
