...
- hubs → rien.
- **Le plugin ne pose AUCUNE meta eco3min** (`_eco3min_level`, `_eco3min_cluster`, `_eco3min_sub_pilier`, `_eco3min_parent_major`). Une page importée reste `uncategorized`, donc **invisible au conseil de maillage du mega**, qui ne traite que `pillar` / `sub_pillar` / `major_article` / `satellite`. Le rattrapage post-import relève de `metas-eco3min`, dans l'ordre : classer le level → poser cluster et sous-pilier → poser `parent_major`. Tenter `parent_major` avant la classification produit une erreur « satellite introuvable ».
