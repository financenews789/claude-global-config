...
### 1.1 Plugins ACTIFS (27 extensions maison, live 15/09/2026)
...
|---|---|---|---|
| **Eco3min Mega** | **1.0.14** | **Cœur du système.** 9 onglets : 📋 Référentiel · 📡 Scan & Classification (scanner read-only + Tinder set/get du level) · 📥 Import cluster (Format C v1.1, batch unique ou multi-pair, drag & drop) · 🔗 Maillage (snapshot JSON + **CSV snapshot/maillage depuis 1.0.14**, exports AVEC/SANS HTML, import patches v1.0.2 chunked, historique + rollback) · 🕸️ Graphique · 📚 Manuel · 🩺 Diagnostic · 🧭 **Conseil maillage T1/T2/T3** (lit `e3m_links`) · 🧹 Réconciliation (conflits slug/ID, suppression réversible des ID). Tables `e3m_*`. | `admin.php?page=eco3min-mega` puis `eco3min-mega-{referentiel,scan,import_cluster,maillage,graph,manuel,diagnostic,conseil,nettoyeur}` |
| **Eco3min Cleanup** | **1.1.0** (déployé le 15/09/2026, bump d'en-tête, code inchangé) | **PRÉ-REQUIS du conseil.** 3 sections, tout réversible : (1) **exclut le bruit** (pages système → `level=exclu`) ; (2) **rattache les satellites orphelins** (export JSON → révision par Claude → import `{"attachments":[{post_id, level?, cluster, sub_pilier, parent_major?}]}` — **`level` accepté en overwrite réversible**, cluster/sub_pilier fill-only, cf §14.8) ; (3) **déclare les hubs** (page 1 segment + enfants) en pilier. | `tools.php?page=eco3min-cleanup` |
| **Subpillar Aligner** | 1.2.0 | Met l'ID `_eco3min_subpillar` en phase avec le slug `_eco3min_sub_pilier` (slug gagne), FR puis miroir EN, verrou avant breadcrumb ID-only. Détail → `metas-eco3min`. | `admin.php?page=eco3min-subpillar-aligner` |
| **Level Setter** | 1.0.0 | Pose `_eco3min_level` sur une liste de post_id (12 levels, **`satellite` absent**), backup `_e3m_lvlset_bak`. | `tools.php?page=eco3min-level-setter` |
...
| **Eco3min MCP** | **1.1.1** (déployé le 15/09/2026 : `set-metas` accepte `cluster` = slug propre sur un pillar, `health` remplace le faux positif « pilier avec meta » par le check `pillar_without_cluster`, 8 checks par défaut) | 14 abilities `eco3min/*` (find, get-content, taxonomy, links, health, set-metas, set-seo, update-content, create-pair, rollback, mailpoet-*). Écriture en dry-run par défaut, rollback. | `tools.php?page=eco3min-mcp` |
| **Eco3min Import** | 2.2.1 | 4 sous-pages : Chart of the Week · Page bilingue · Import dataset · MAJ metas (metas `_eco3min_*` + RankMath sur pages existantes, overwrite réversible, dry run). Bundles → `eco3min-import-contenu-bilingue`. | `admin.php?page=eco3min-import{,-page,-dataset,-metas}` |
...
| **Polylang Repair** | 1.0.3 | Audit + réparation du mapping Polylang des 80 pages Q&A des batches 7/08/32/33, uniquement là où absent. | `admin.php?page=eco3min-polylang-repair` |
...
1. **Classer** les `uncategorized` → Tinder du mega (onglet 📡 Scan & Classification ; metas vides uniquement), ou Cleanup import avec `level`, ou Level Setter (pas `satellite`).
...
9. **Re-scan**, puis Diagnostic + Conseil ; classifier les satellites livrés en `uncategorized`.
...
## 12. Mega-plugin Eco3min 1.0.14 (lire `references/03-mega-versions-immutabilite-format-c.md`)
...
- **Format C v1.1 (12.5), règles strictes à l'import** : Polylang actif ; slug FR + slug EN uniques dans le batch ET en BDD, slug FR ≠ slug EN ; levels valides parmi `pillar, sub_pillar, major_article, foundation_article, case_study, deep_study, dataset, faq, tool, beginner, uncategorized` ; liens FR vers `https://eco3min.fr/SLUG-FR/`, EN vers `https://eco3min.fr/en/SLUG-EN/`, pas de cross-lang ; `wp_category_slug` existant en BDD ; `parent_major_pair_id` déclaré AVANT le satellite qui le référence ; `expected_pairs_count` = `len(articles)` ; `rank_math_title` ≤ 60, `rank_math_description` ≤ 155.
...
Bloquant :
- `ewpa/create-code-snippet` **crée un nouveau snippet, toujours inactif, et ne met à jour aucun snippet existant** : s'en servir pour patcher produit un doublon inactif sans erreur. Le déploiement d'un patch de snippet est manuel, par l'interface Code Snippets ; un projet livre le patch, quelqu'un le colle.
- `~/eco3min-wp/snippets/` est un **export** : relire la version en base dans wp-admin avant de proposer un patch.
